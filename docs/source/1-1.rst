...
In this tutorial, we are going to work with a specific version of ROS called Kinetic. Also, some ROS packages are needed in order to perform the simulatoin excercises mentioned in this tutorial. The following sections will guide you throught the installation procedures.
...
Simply follow the instructions in the official ROS wiki to install ROS Kinetic - http://wiki.ros.org/kinetic/Installation/Ubuntu.
...
For reference see: http://wiki.ros.org/Robots/TurtleBot
...
.. code-block:: bash
  ...
  # make the python file execuatble
  chmod +x simple.py
...
.. hint::
  ...
  In this case, the robot uses a differential drive plugin to move. That is, the robot can only move linearly in the 'x' axis, or rotationaly in the angular z axis. This means that the only values that you need to fill in the Twist message are the linear x and the angular z.
