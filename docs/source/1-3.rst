...
* Depending on the receiver you use, follow the following steps.
...
receivers connect to the flight controller using the port appropriate to their supported protocol:
  ...
  1. Insert the jumper wire to Pixhawk like a motor, we just need to power it through this way
  2. Insert the signal wire to another end of the RC receiver
  3. Plug the battery (Find in the toolbox) to power wire
  4. Turn on the RC while pressing the left up button (marked as Trainer Bind), wait until the RC receiver is lighting statically
  5. Remove the RC from binder, use its own signal wire to connect with Pixhawk via port SPKT/DSM
